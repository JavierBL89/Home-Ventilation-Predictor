...
def prophet_model(df):
    ...
    model = Prophet()
    model.add_regressor("outdoor_temperature")
    model.fit(prophet_df)
    return model   # 👈 devolvemos el modelo entrenado
...
def simulate_indoor_temperature(df):
    hours = df.index.hour
    daily_cycle = 2 * np.sin(2 * np.pi * hours / 24)  # variación diaria
    noise = np.random.normal(loc=0, scale=0.5, size=len(df))
    return df["outdoor_temperature"] + daily_cycle + noise
